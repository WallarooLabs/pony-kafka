...
actor Main is KafkaClientManager
  ...
  new create(env: Env) =>
    ...
    let kconf =
      recover iso
        let kc = KafkaConfig(_logger, "My Client" where use_java_compatible_snappy_compression' = false)
        kc.add_broker("127.0.0.1", 9092)
// uncomment for producer only config
//        kc.add_topic_config("test")

// uncomment for consumer only config and tell kafka to send messages to consumers
//        kc.add_topic_config("test", KafkaConsumeOnly where consumer_message_handler = recover KafkaRoundRobinConsumerMessageHandler end, compression = KafkaGzipTopicCompression)

// producer/consumer config and tell kafka to send messages to consumers
        kc.add_topic_config("test", KafkaProduceAndConsume where consumer_message_handler = recover KafkaRoundRobinConsumerMessageHandler end, compression = KafkaSnappyTopicCompression)
...
actor C is KafkaConsumer
  ...
  // behavior kafka calls for each message received that should be sent to this actor
  be receive_kafka_message(msg: KafkaMessage val, network_received_timestamp: U64) =>
    ...
    logger.log(Info, "CONSUMER(" + _name + ")-MSG(" + _i.string() + "): Received Msg. topic: " + msg.get_topic() + ", partition: " + msg.get_partition_id().string() + ", offset: " + msg.get_offset().string() + ", value: " + m)
...
actor P is KafkaProducer
  // variable to hold producer mapping for sending requests to broker connections
  ...
  be kafka_message_delivery_report(delivery_report: KafkaProducerDeliveryReport) =>
    if not (delivery_report.status is ErrorNone) then
      logger(Error) and logger.log(Error, "received delivery report: " + delivery_report.string())
    else
      logger(Fine) and logger.log(Fine, "received delivery report: " + delivery_report.string())
    end
  ...
  fun ref produce_data() =>
    ...
    try
      let ret = (_kafka_producer_mapping as KafkaProducerMapping ref).send_topic_messages("test", d)
    ...
    for (o, v, k) in d2.values() do
      ...
      try
        let ret = (_kafka_producer_mapping as KafkaProducerMapping ref).send_topic_message("test", o, v, k)
        if ret isnt None then error end
      else
        logger(Error) and logger.log(Error, "error sending messages to brokers")
      end
  ...
  // generate data
  fun ref generate_data(start: USize = 0): Array[(Any tag, (ByteSeq | Array[ByteSeq] val), (None | ByteSeq | Array[ByteSeq] val))] =>
    let msgs = Array[(Any tag, (ByteSeq | Array[ByteSeq] val), (None | ByteSeq | Array[ByteSeq] val))]
